...
export default function LandingPage() {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        ...
        console.error(e);
        if (!cancelled) setLoadError(e?.message || 'فشل تحميل البيانات');
      } finally {
  ...
  return (
    <main className="min-h-screen bg-page selection:bg-gold/30 selection:text-gold">
      <Navbar onBookNow={() => setShowBooking(true)} />
      {loadError && (
        <div className="mx-4 mt-24 rounded-xl border border-red-500/30 bg-red-500/10 px-4 py-3 text-center text-sm text-red-600 dark:text-red-400 font-arabic">
          {loadError}
        </div>
      )}
      <Hero images={heroSlides} />
      <BentoGrid
        hotels={hotels}
        loading={loading}
        onSelectHotel={(hotel) => navigate(`/hotel/${hotel.id}`)}
      />

      <footer id="contact" className="relative py-16 md:py-28 border-t border-ink/[0.06] z-10">
        <div className="max-w-4xl mx-auto px-4">
          ...
          <div className="flex flex-col sm:flex-row items-center justify-center gap-6 sm:gap-10 mb-12">
            ...
            <div className="hidden sm:block w-px h-10 bg-ink/[0.06]" />

            <a href="tel:0222910609" className="flex items-center gap-3 group">
              ...
              <div className="text-right">
                <span className="block text-muted text-xs font-arabic">حجز إسكان - دار ضباط المدفعية</span>
                <span className="block text-ink font-inter text-sm tracking-wider ltr" dir="ltr">0222910609</span>
              </div>
            </a>

            <div className="hidden sm:block w-px h-10 bg-ink/[0.06]" />

            <a href="tel:+201505346563" className="flex items-center gap-3 group">
              <div className="w-10 h-10 rounded-full bg-gold/10 flex items-center justify-center group-hover:bg-gold/20 transition-colors duration-300">
                <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="w-5 h-5 text-gold">
                  <path d="M22 16.92v3a2 2 0 0 1-2.18 2 19.79 19.79 0 0 1-8.63-3.07 19.5 19.5 0 0 1-6-6 19.79 19.79 0 0 1-3.07-8.67A2 2 0 0 1 4.11 2h3a2 2 0 0 1 2 1.72 12.84 12.84 0 0 0 .7 2.81 2 2 0 0 1-.45 2.11L8.09 9.91a16 16 0 0 0 6 6l1.27-1.27a2 2 0 0 1 2.11-.45 12.84 12.84 0 0 0 2.81.7A2 2 0 0 1 22 16.92z"/>
                </svg>
              ...
              <div className="text-right">
                <span className="block text-muted text-xs font-arabic">رقم فندق كينج توت</span>
                <span className="block text-ink font-inter text-sm tracking-wider ltr" dir="ltr">+20 15 05346563</span>
              </div>
            </a>
          </div>
          ...
          <div className="text-center space-y-4">
            ...
            <p className="text-faint text-sm font-arabic">
